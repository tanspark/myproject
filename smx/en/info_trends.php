...
$pageSize = 5;
$total = MySqlOperate::getInstance()->totals('t_trends');
$pageTotal = (int)(($total % $pageSize == 0) ? ($total / $pageSize) : ($total / $pageSize + 1));
$res = MySqlOperate::getInstance()->field('t_id,t_title,t_desc,t_author,t_time')
            ...
            <div class="row">
                ...
                <div class="col s9 m9 l9">
                    <div class="fav_list">
                        <div data-v-357a65ed="" class="fav_list_box">
                            <div  class="fav_list_title">
                                <h3 class="fav_list_title_h3">Trend</h3>
                            </div>
                                ...
                                <ul  class="my_fav_list">
                                    <?php if(!empty($res)) for ($i=0; $i < count($res) ; $i++) { ?>
                                        <li class="my_fav_list_li" style="width: 100%;">
                                            <p style="float: left; width: 70%;"><a class="my_fav_list_a" href="info_trends_detail.php?id=<?php echo $res[$i]['t_id']; ?>">
                    ...
                    <ul class="pagination w100dt">
                        <?php if($pageNo > 1) { ?>
                            <li class="waves-effect"><a href="info_trends.php?pageNo=<?php echo $pageNo-1;?>"><i class="icofont icofont-simple-left"></i></a></li>
                        <?php } ?>
                        <?php
                        for( $i=0; $i < $pageTotal; $i++) {
                            ?>
                            <li <?php if($pageNo==$i+1){ ?>class="active"<?php } ?> >
                                <a href="info_trends.php?pageNo=<?php echo $i+1;?>"><?php echo $i+1; ?></a>
                            </li>
                        <?php }?>
                        <?php if($pageNo < $pageTotal) { ?>
                            <li class="waves-effect"><a href="info_trends.php?pageNo=<?php echo $pageNo+1;?>"><i class="icofont icofont-simple-right"></i></a></li>
                        <?php } ?>
                    </ul>
