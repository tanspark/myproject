...
if($_GET['type'] == 1 or $_GET['type'] == 2) {
    $page_link = 'join_award.php';
    $page_sub_name = "Achievement Competition";
} else if($_GET['type'] == 3 or $_GET['type'] == 4) {
    $page_link = 'join_award.php';
    $page_sub_name = "Creative Competition";
}
...
$res = MySqlOperate::getInstance()->field('t_id,t_title,t_type,t_content,t_time')
    ->order('t_time desc')
    ->where('t_type='.$_GET['type'].' and t_selected=1')
