...
class MySqlOperateMatch
{
    ...
    protected $_pass = '123123';
    protected $_dbName = "t_smx_en"; //数据库名
    ...
    {
        $conf = array("host"=>"127.0.0.1", "port"=>"3306", "user" => "root", "passwd"=>"123123", "dbname"=>"t_smx_en");
    ...
    /**
     * 插入方法
     * @param string $tbName 操作的数据表名，带上当前时间
     * @param array $data 字段-值的一维数组
     * @return int 受影响的行数
     */
    public function insertAndTime($tbName, array $data)
    {
        $data = $this->_dataFormat($tbName, $data);
        if (!$data) return;
        $sql = "insert into " . $tbName . "(" . implode(',', array_keys($data)) . ", t_time ) values(" . implode(',', array_values($data)) . ", now())";
        return $this->_doExec($sql);
    }
     ...
     * @return int 结果集
     */
    public function totals($tbName = '')
    {
        $sql = "select "." * "." from " . $tbName . " ";
        $this->_clear = 1;
        $this->_clear();
        return count($this->_doQuery(trim($sql)));
    }
